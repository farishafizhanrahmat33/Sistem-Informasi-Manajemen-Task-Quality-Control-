...
def login():
    ...
    if user and check_password_hash(user.password, password):
        ...
        return jsonify({
            "status": "success",
            "message": f"You're in! Logged in as {user.role}."
        })
    else:
        return jsonify({
            "status": "error",
...
def profile():
    ...
    if not user_id:
        flash('Log in first to check out your profile.', 'warning')
        return redirect(url_for('main.dashboard'))
...
def edit_profile():
    ...
    if user:
        ...
        if new_username and new_username != user.username:
            ...
            if existing_user:
                flash('Nope, that username is already taken by someone else!', 'danger')
...
def change_password():
    ...
    if user and check_password_hash(user.password, old_password):
        ...
        db.session.commit()
        flash('Password changed, nice!', 'success')
    else:
...
def support():
    ...
    if request.method == 'POST':
        ...
        flash('Your message got sent over to the Developer!', 'success')
...
def view_tickets():
    ...
    if current_role != 'Developer':
        flash('Access denied! This support inbox is Developer-only.', 'danger')
        return redirect(url_for('main.dashboard'))
...
def toggle_ticket_status(ticket_id):
    ...
    if ticket:
        ...
        db.session.commit()
        flash(f'Ticket marked as {ticket.status}.', 'info')
    else:
...
def user_management():
    if session.get('role') != 'Developer':
        flash('Access denied! This page is Developer-only.', 'danger')
        return redirect(url_for('main.dashboard'))
...
def add_user():
    ...
    if existing:
        flash('Nope, that username is already taken!', 'danger')
    else:
        ...
        db.session.commit()
        flash(f'Account {username} created with the {role} role!', 'success')
...
def delete_user(user_id):
    ...
    if user:
        if user.id == session.get('user_id'):
            flash("Can't delete the account you're currently logged in with!", 'danger')
        else:
...
def update_user_role(user_id):
    if session.get('role') != 'Developer':
        flash('Access denied! Only Developers can change permissions.', 'danger')
        return redirect(url_for('main.dashboard'))
    ...
    try:
        ...
        db.session.rollback()
        flash('Something went wrong updating the role.', 'danger')
